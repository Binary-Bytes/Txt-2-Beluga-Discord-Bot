...
# Load fonts
font = "ggsans" # Change this according to the font you want to use
name_font = ImageFont.truetype(os.path.join(f'src/t2b/assets/fonts/{font}', 'semibold.ttf'), NAME_FONT_SIZE)
time_font = ImageFont.truetype(os.path.join(f'src/t2b/assets/fonts/{font}', 'semibold.ttf'), TIME_FONT_SIZE)
message_font = ImageFont.truetype(os.path.join(f'src/t2b/assets/fonts/{font}', 'medium.ttf'), MESSAGE_FONT_SIZE)
message_italic_font = ImageFont.truetype(os.path.join(f'src/t2b/assets/fonts/{font}', 'medium_italic.ttf'), MESSAGE_FONT_SIZE)
message_bold_font = ImageFont.truetype(os.path.join(f'src/t2b/assets/fonts/{font}', 'bold.ttf'), MESSAGE_FONT_SIZE)
message_italic_bold_font = ImageFont.truetype(os.path.join(f'src/t2b/assets/fonts/{font}', 'bold_italic.ttf'), MESSAGE_FONT_SIZE)
message_mention_font = ImageFont.truetype(os.path.join(f'src/t2b/assets/fonts/{font}', 'semibold.ttf'), MESSAGE_FONT_SIZE)
message_mention_italic_font = ImageFont.truetype(os.path.join(f'src/t2b/assets/fonts/{font}', 'semibold_italic.ttf'), MESSAGE_FONT_SIZE)

# Load profile picture dictionary
with open('src/t2b/assets/profile_pictures/characters.json', encoding="utf8") as file:
...
def generate_joined_message(name, time, template_str, arrow_x, color=NAME_FONT_COLOR):
    ...
    arrow = Image.open("src/t2b/assets/green_arrow.png")
...
def save_images(chat_folder, lines, init_time, dt=30):
    ...
    for line in lines:
        ...
        if line.startswith("WELCOME "):
            ...
            image.save(os.path.join(chat_folder, f'{msg_number:03d}.png'))
        ...
        image = generate_chat(
            ...
            name_time=name_time,
            profpic_file=os.path.join('src/t2b/assets/profile_pictures', characters_dict[current_name]["profile_pic"]),
...
if __name__ == '__main__':
    vid_id = sys.argv[2]
    folder_path = os.path.join('./', vid_id)
    ...
    chat_folder = os.path.join(folder_path, 'chat')
    save_images(chat_folder, lines, init_time=current_time)

    from compile_images import gen_vid
    gen_vid(sys.argv[1], folder_path, vid_id)